...
const ProductGrid = ({ categories, maxPrice, availabilities, searchQuery, onAddToCart, onRemoveFromCart }: ProductGridProps) => {
    const filtered = products
        .filter(p => categories.includes("all") || categories.includes(p.category.toLowerCase()))
        .filter(p => p.price <= maxPrice)
        .filter(p => availabilities.length === 0 ||
            (availabilities.includes("in-stock") && p.stockStatus.toLowerCase().includes("in stock")) ||
            (availabilities.includes("on-sale") && p.discountPrice !== undefined)
        )
